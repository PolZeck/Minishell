...
/*   Updated: 2025/05/19 13:48:26 by pol              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	run_builtin(t_cmd *cmd, t_data *data)
{
	...
	if (ft_strcmp(cmd->args[0], ".") == 0)
	{
		...
		data->exit_status = builtin_cd(cmd, data);
	else if (ft_strcmp(cmd->args[0], "echo") == 0)
		data->exit_status = builtin_echo(cmd);
	else if (ft_strcmp(cmd->args[0], "exit") == 0)
		data->exit_status = builtin_exit(cmd, data);
	else if (ft_strcmp(cmd->args[0], "pwd") == 0)
		data->exit_status = builtin_pwd(cmd, data);
	else if (ft_strcmp(cmd->args[0], "export") == 0)
		data->exit_status = builtin_export(cmd, data);
	else if (ft_strcmp(cmd->args[0], "unset") == 0)
		data->exit_status = builtin_unset(cmd, data);
	else if (ft_strcmp(cmd->args[0], "env") == 0)
		data->exit_status = builtin_env(cmd, data);
}
...
void	execute_builtin(t_cmd *cmd, t_data *data)
{
	...
	if (apply_redirs(cmd->redirs, data))
	{
